...
class SnapKVCluster():
    def __init__(self, window_size = 64, max_capacity_prompt = 256 + 64, kernel_size = 5, pooling = 'avgpool', layer_idx = None, num_hidden_layers = None, pyram_mode = False, pyram_beta = 20,gqa_support=False,num_key_value_groups = 1, gqa_func=None):
        ...
        if not isinstance(self.max_capacity_prompt, float):
    ...
    def update_kv(self, origin_key_states, query_states, origin_value_states):
        ...
        # Support percentage budget
        current_max_capacity = self.max_capacity_prompt
        if isinstance(current_max_capacity, float) and current_max_capacity <= 1.0:
            current_max_capacity = int(q_len * current_max_capacity)
        ...
        if self.pyram_mode and not self.pyram_init:
            ...
            base_capacity = current_max_capacity - self.window_size
            ...
            current_max_capacity = max_num - self.layer_idx * steps + self.window_size
            self.max_capacity_prompt = current_max_capacity # Update stored value for consistency if needed, though local is safer
            self.pyram_init = True
            print(f"Pyram mode adaptive capacity, layer: {self.layer_idx}, max_capacity_prompt: {current_max_capacity}, base_capacity: {current_max_capacity - self.window_size}", flush=True)
        ...
        if q_len < current_max_capacity:
            ...
            indices = attn_cache.topk(current_max_capacity - self.window_size, dim=-1).indices
            
            # import json
            # with open('./adakv_snapkv.jsonl', 'a') as f:
            #     f.write(json.dumps({
            #         'layer_idx': self.layer_idx,
            #         'top_token_idx': indices.cpu().numpy().tolist()
            #     }) + '\n')
...
class AdaptiveSnapKVCluster():
    def __init__(self, window_size = 32, kernel_size = 7, pooling = 'maxpool',base_capacity=None,floor_alpha = None,skip = None,normalize=None, 
        ...
        self.pooling = pooling
        # Support percentage budget
        self.raw_base_capacity = base_capacity
        if isinstance(base_capacity, (int, torch.Tensor)):
            self.base_capacity = base_capacity - window_size
        else:
            # For float (percentage), we will calculate it during update_kv
            self.base_capacity = base_capacity 

        self.floor_ratio = floor_alpha
        # These will be updated in update_kv if base_capacity is float
        if not isinstance(self.base_capacity, float):
    ...
    def update_kv_gqa(self, origin_key_states, query_states, origin_value_states):
        ...
        # Support percentage budget
        if isinstance(self.base_capacity, float) and self.base_capacity <= 1.0:
            actual_base_capacity = int(q_len * self.base_capacity) - self.window_size
            actual_base_capacity = max(actual_base_capacity, 1)
            self.floor_capacity = int(actual_base_capacity * self.floor_ratio)
            self.adaptive_capacity = actual_base_capacity - self.floor_capacity
            current_base_capacity = actual_base_capacity
        else:
            current_base_capacity = self.base_capacity
        ...
        if self.pyram_mode and not self.pyram_init:
            # NOTE: (max_num + min_num) / 2 == base_capacity to restrict the total capacity
            min_num = current_base_capacity // self.pyram_beta
            max_num = current_base_capacity * 2 - min_num
                
            # if the max_num is larger than the query length, we need to adjust the max_num
            if max_num >= q_len - self.window_size:
                max_num = q_len - self.window_size
                min_num = current_base_capacity * 2 - max_num
        
            # NOTE: compute interval
            steps = (max_num - min_num) // (self.num_hidden_layers - 1)

            # renew adaptive capacity
            current_base_capacity = max_num - self.layer_idx * steps
            self.floor_capacity = int(current_base_capacity * self.floor_ratio)
            self.adaptive_capacity = current_base_capacity - self.floor_capacity
            self.pyram_init = True
            print(f"Pyram mode adaptive capacity, layer: {self.layer_idx}, acap: {self.adaptive_capacity}, bcap: {current_base_capacity}, fcap: {self.floor_capacity}",  flush=True)
        ...
        if current_base_capacity > attn_score.size(-1):
        ...
        if self.layer_idx >= self.skip:
            ...
            if self.normalize:
                ratio_weight = sorted_attn_score[...,:current_base_capacity].sum(dim=-1,keepdim=True)/sorted_attn_score.sum(dim=-1,keepdim=True)
            ...
            sorted_indices = torch.topk(adaptive_attn_score,k=num_heads*current_base_capacity//self.num_key_value_groups,dim=-1).indices
            ...
            assert head_adaptive_capacity.sum().item() == num_heads*current_base_capacity//self.num_key_value_groups
            head_adaptive_capacity = torch.round(head_adaptive_capacity * (1-self.floor_ratio) + self.floor_capacity).int()
        else:
            head_adaptive_capacity = torch.ones((bsz,num_heads),device=_device,dtype = sorted_attn_score_indices.dtype) * current_base_capacity
        ...
        for head_idx in range(num_heads//self.num_key_value_groups):
            ...
            # collected_indices.append(cache_index.cpu().numpy().tolist())
        ...
        #     f.write(json.dumps({
        #         'layer_idx': self.layer_idx,
        #         'top_token_idx': collected_indices
        #     }) + '\n')
    ...
    def update_kv_wo_gqa(self,  origin_key_states, query_states, origin_value_states):
        ...
        if self.layer_idx >= self.skip:
            ...
            sorted_indices = torch.topk(adaptive_attn_score,k=num_heads*current_base_capacity,dim=-1).indices
            sorted_indices = sorted_indices//length
            # floor_alpha capacity set
            head_adaptive_capacity = torch.zeros((bsz,num_heads),device=_device,dtype = sorted_indices.dtype)
            head_adaptive_capacity.scatter_add_(-1,sorted_indices,torch.ones_like(sorted_indices,dtype=head_adaptive_capacity.dtype),)
            assert head_adaptive_capacity.sum().item() == num_heads*current_base_capacity
            head_adaptive_capacity = torch.round(head_adaptive_capacity * (1-self.floor_ratio) + self.floor_capacity).int()
        else:
            head_adaptive_capacity = torch.ones((bsz,num_heads),device=_device,dtype = sorted_attn_score_indices.dtype) * current_base_capacity
        sorted_attn_score_indices = sorted_attn_score_indices.split(1,dim=1)
...
class StreamingLLMKVCluster():
    def __init__(self, max_capacity_prompt = 256):
        self.sink_token = 4
        if isinstance(max_capacity_prompt, float):
            self.max_capacity_prompt = max_capacity_prompt
        else:
            self.max_capacity_prompt = max_capacity_prompt - self.sink_token
    ...
    def update_kv(self, key_states, query_states, value_states, *args, **kwargs):
        ...
        if isinstance(current_max_capacity, float) and current_max_capacity <= 1.0:
            ...
            # For fixed capacity, current_max_capacity is already (total - sink)
            current_max_capacity = current_max_capacity + self.sink_token

        print(f"StreamingLLM max_capacity_prompt {current_max_capacity}")
        ...
        if q_len < current_max_capacity:
            ...
            # We keep the latest tokens, the number of tokens is (current_max_capacity - sink_token)
            keep_len = current_max_capacity - self.sink_token
            k_cur = key_states[:, :, -keep_len:, :]
            v_cur = value_states[:, :, -keep_len:, :]
            key_states = torch.cat([k_past_compress, k_cur], dim = 2)
            value_states = torch.cat([v_past_compress, v_cur], dim = 2)
            return key_states, value_states
